...
func EncodeCapsule(capsule Capsule) (capsuleAsBytes []byte, err error) {
	...
	if err = enc.Encode(capsule); err != nil {
		// fmt.Println("EncodeCapsule!!!")
		return nil, err
	}
	return buf.Bytes(), nil
}

func Mssong_EncodeCapsule(capsule Capsule) (capsuleAsBytes []byte, err error) {

	Ebytes, err := x509.MarshalPKIXPublicKey(capsule.E)
	if err != nil {
		panic(err)
	}
	Vbytes, err := x509.MarshalPKIXPublicKey(capsule.V)
	if err != nil {
		panic(err)
	}
	...
	// result = append(result, sizeSbyte)
...
func Mssong_DecodeCapsule(capsuleAsBytes []byte) (capsule *Capsule, err error) {
	sizeEbyte := int(capsuleAsBytes[0])
	sizeVbyte := int(capsuleAsBytes[1])
	// sizeSbyte := int(capsuleAsBytes[2])

	startE := 2
	startV := startE + sizeEbyte
	startS := startV + sizeVbyte

	E_any, err := x509.ParsePKIXPublicKey(capsuleAsBytes[startE:startV])
	E := E_any.(*ecdsa.PublicKey)
	V_any, err := x509.ParsePKIXPublicKey(capsuleAsBytes[startV:startS])
	V := V_any.(*ecdsa.PublicKey)
	S := new(big.Int)
	S.SetBytes(capsuleAsBytes[startS:len(capsuleAsBytes)])
...
// clientPubKey, err := x509.ParsePKIXPublicKey([]byte(clientPubKeyString))
// 	if err != nil {
// 		// log.Fatalf("Failed to Unmarshal clientPubKey: %v", err)
// 		panic(err)
// 	}
// 	a := clientPubKey.(*ecdsa.PublicKey)
