...
// https://github.com/SherLzp/goRecrypt
func main() {
	...
	aPriKeyBytes, _ := json.Marshal(aPriKey)
	bPubKeyBytes, _ := json.Marshal(bPubKey)
	fmt.Println("size newCapsule, pubX, cipherText:", len(newCapsuleBytes), len(pubXBytes), len(cipherText), len(aPriKeyBytes), len(bPubKeyBytes))
